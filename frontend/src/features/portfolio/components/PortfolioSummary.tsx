...
export function PortfolioSummary({ totalAsset, totalPurchaseAmount, totalProfit, totalReturnRate }: Props) {
    const isPlus = totalProfit >= 0;
    ...
    return (
      ...
      <Card className="border-none shadow-md bg-gradient-to-br from-gray-900 to-gray-800 text-white">
        <CardContent className="p-6 flex flex-col justify-between h-full">
          <div>
            ...
            <h2 className="text-3xl font-bold mt-2">{formatKrw(totalAsset)}</h2>
          </div>
      ...
      <Card className="border-none shadow-sm bg-white">
        <CardContent className="p-6 flex flex-col justify-center h-full">
          ...
          <div className={`text-2xl font-bold mt-2 flex items-center gap-1 ${colorClass}`}>
            ...
            {isPlus ? "+" : ""}{formatKrw(totalProfit)}
          </div>
          <p className={`text-sm mt-1 font-medium ${colorClass}`}>
            {isPlus ? "+" : ""}{(totalReturnRate * 100).toFixed(2)}%
          </p>
        </CardContent>
      </Card>
      ...
      <Card className="border-none shadow-sm bg-white">
        <CardContent className="p-6 flex flex-col justify-center h-full">
          ...
          <h2 className="text-2xl font-bold mt-2 text-gray-800">{formatKrw(totalPurchaseAmount)}</h2>
