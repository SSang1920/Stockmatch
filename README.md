...
- **Redis Cache-Aside 캐싱**: 재무 데이터 및 반복 요청에 대한 캐시 전략으로 외부 금융 API 호출 건수 최적화 및 비용 절감
- **AI 인프라 안정성 확보**: Structured Outputs(Strict Schema) 및 `AiRequestGuard`를 통한 신뢰성·비용 통제
